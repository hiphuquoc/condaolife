@if(!empty($item->options)&&$item->options->isNotEmpty())
    <div class="contentShip_item">
        ...
        <div class="contentTour_item_text">
            <p>
                <strong>Bảng giá vé</strong> sẽ được cập nhật liên tục theo chính sách của các hãng vui chơi, đơn vị tổ chức. Đây là bảng giá tham khảo áp dụng cho khách lẻ. Đối với khách đoàn lớn (20 khách trở lên) và đối tác vui lòng liện hệ <a href="tel:{{ \App\Helpers\Charactor::removeSpecialCharacters(config('main.company.hotline')) }}"><span style="font-size:1.4rem;font-weight:bold;color:rgb(0,123,255);">{{ config('main.company.hotline') }}</span></a> để biết thêm chi tiết.
            </p>
