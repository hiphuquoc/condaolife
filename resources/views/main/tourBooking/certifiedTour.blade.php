<div class="certifiedBox">
    <div class="certifiedBox_image">
        <img src="/images/main/icon-certified.png" alt="chứng nhận {{ config('main.name') }} đại lý uy tín" title="chứng nhận {{ config('main.name') }} đại lý uy tín" />
    </div>
    <div class="certifiedBox_content">
        <div class="certifiedBox_content_title">
            Danh hiệu {{ config('main.name') }} được khách hàng công nhận!
        </div>
        <div class="certifiedBox_content_list">
            <div>
                Đơn vị cung cấp Tour uy tín, chất lượng.
            </div>
            <div> 
                Hệ thống đặt Tour 2 bước tiện lợi, nhanh chóng.
            </div>
            <div> 
                Hỗ trợ chu đáo trước và sau quá trình tham gia Tour.
            </div>
        </div>
    </div>
</div>