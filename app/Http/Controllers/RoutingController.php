        ...
        if(!empty($checkExists->type)){
            ...
            if(file_exists($pathCache)&&$cacheTime>(time() - filectime($pathCache))){
                ...
                if($checkExists->type=='page_info'){
                    ...
                    $xhtml              = view('main.page.index', compact('item', 'breadcrumb', 'partners', 'content'))->render();
                }
                /* ===== TOUR PAGE INFO */
                if($checkExists->type=='feedback_info'){
                    $flagMatch          = true;
                    ...
                    $feedbacks          = Feedback::select('*')
                                            ...
                                            ->where('id', '!=', $item->id)
                                            ->get();
                    $xhtml              = view('main.feedback.index', compact('item', 'breadcrumb', 'partners', 'feedbacks'))->render();
                }
                /* ===== COMBO LOCATION */
