...
class ConfirmShipBooking implements ShouldQueue
{
    ...
    public function handle(){
        ...
        if($this->typeEmail=='notice'){
            ...
            $titleMail          = config('main.webname').' - Xác nhận đặt vé '.$this->infoShipBooking->infoDeparture[0]->departure.' - '.$this->infoShipBooking->infoDeparture[0]->location.$typeDeparture.' số '.$this->infoShipBooking->no.' - '.date('H:i d/m/Y', time());
            $bodyMail           = view('admin.shipBooking.confirmBooking', [
                'item' => $this->infoShipBooking, 
                'infoStaff' => $this->infoStaff
            ]);
